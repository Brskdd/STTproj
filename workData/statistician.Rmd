...
```  
```{r}  
plot(speedDatingCleanedQ3$AgeM, speedDatingCleanedQ3$AttractiveM )
cor(speedDatingCleanedQ3$AgeM, speedDatingCleanedQ3$AttractiveM)

plot(speedDatingCleanedQ3$AgeF, speedDatingCleanedQ3$AttractiveF )
cor(speedDatingCleanedQ3$AgeF, speedDatingCleanedQ3$AttractiveF)
```
...
There is a strong positive relationship between ratings for Attractiveness and 
Fun for both genders.

The p-value for both corrolations is 1. The statistics are significant, there 
is significant evidence that Attractiveness and Fund are corrolated for both
men and women.
