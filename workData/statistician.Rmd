...
hist(nullDistribution,  xlab = "Difference in decision (M-F)", main = "Simulated Difference of Saying Yes between Men and Women")
abline(v = statistic, col = 'red')
```
With a p-value of 0.052 we fail to reject the null hypothesis with a statistic of 0.068 difference between males and females saying yes to a second date. We do not have significant evidence that supports the alternative hypothesis that males say yes to second dates more than females.


## Q2.a Which score seems to be the most important for a man's Like score?
...
cor(speedDatingCleanedQ2M[sapply(speedDatingCleanedQ2M, is.numeric)])
```
```{r}

corr_matrix <- cor(speedDatingCleanedQ2M[sapply(speedDatingCleanedQ2M, is.numeric)], 
                   use = "complete.obs")
theplot<- ggcorrplot(corr_matrix, method = "circle", type = "upper",
           lab = TRUE, lab_size = 3) +  
  labs(title = "Correlation Matrix of Speed Dating Variables (Male)")
ggsave("Q2_corrolation_M.png", plot = theplot, width = 6, height = 4, units = "in", dpi = 300)
theplot

```
The none of the corrolations are stonger than 0.6 so we can go ahead with linear regression.
Age and race do not seem to have a relationship with like rating.

## Q2.b Which score seems to be the most important for a woman's Like score?
...
cor(speedDatingCleanedQ2F[sapply(speedDatingCleanedQ2F, is.numeric)])
```
```{r}

corr_matrix <- cor(speedDatingCleanedQ2F[sapply(speedDatingCleanedQ2F, is.numeric)], 
                   use = "complete.obs")
theplot <- ggcorrplot(corr_matrix, method = "circle", type = "upper",
           lab = TRUE, lab_size = 3) +  
  labs(title = "Correlation Matrix of Speed Dating Variables (Female)")
ggsave("Q2_corrolation_F.png",  plot = theplot, width = 6, height = 4, units = "in", dpi = 300)
theplot
```
There seems to be The none of the corrolations are stonger than 0.6 so we can go ahead with linear regression.
Age and race do not seem to have a relationship with like rating.
...
```{r}  
plot(speedDatingCleanedQ3$ageDiff, speedDatingCleanedQ3$AttractiveM, xlab = " Age Gap (M-F)", ylab = "Attractiveness Rating", main = "Relationship between percieved Attractiveness and Fun (Male)")
...
hist(nullDistributionM, xlab = "Correlation Coefficient", main = "Simulated Correlation between Age Gap and Attractiveness (Male)")
...
```{r}
plot(speedDatingCleanedQ3$ageDiff, speedDatingCleanedQ3$AttractiveF, xlab = " Age Gap (M-F)", ylab = "Attractiveness Rating", main = "Relationship between percieved Attractiveness and Fun (Female)")
...
hist(nullDistributionF, xlab = "Correlation Coefficient", main = "Simulated Correlation between Age Gap and Attractiveness (Female)")
...
```{r}
plot(speedDatingCleanedQ4$AttractiveM, speedDatingCleanedQ4$FunM, xlab = "Attractiveness Rating", ylab = "Fun Rating", main = "Relationship between percieved Attractiveness and Fun (Male)")
...
hist(nullDistributionM, xlim = c(-0.2, 0.8), xlab = "Correlation Coefficient", main = "Simulated Correlation between Attractiveness and Fun (Male)")
abline(v = statisticM, col = 'red')
```

```{r}

plot(speedDatingCleanedQ4$AttractiveF, speedDatingCleanedQ4$FunF, xlab = "Attractiveness Rating", ylab = "Fun Rating", main = "Relationship between percieved Attractiveness and Fun (Female)")
...
hist(nullDistributionF, xlim = c(-0.2, 0.6), xlab = "Correlation Coefficient", main = "Simulated Correlation between Attractiveness and Fun (Female)")
abline(v = statisticF, col = 'red')
```
